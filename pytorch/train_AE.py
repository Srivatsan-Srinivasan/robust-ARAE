# -*- coding: utf-8 -*-
"""
Created on Sun Apr 22 00:09:10 2018
...
#                errG = train_gan_g(gan_gen, gan_disc, optimizer_gan_g, args, writer, niter_global + (-1 + epoch) * len(train_data))
        ...
        tensorboard(niter_global + (-1 + epoch) * len(train_data), writer, MLP_G(), MLP_D(), autoencoder, args.tensorboard_freq) if args.n_gpus == 1 else tensorboard(niter_global, writer, MLP_G().module, MLP_D().module, autoencoder.module,
...
#                evaluate_generator(gan_gen, autoencoder, corpus, fixed_noise, "epoch{}_step{}".format(epoch, niter_global), args)  # evaluate_generator(fixed_noise, "epoch{}_step{}".format(epoch, niter_global))
                ...
                if not args.no_earlystopping and epoch > args.min_epochs and allow_termination:
                    ...
                    if best_ppl is None or ppl < best_ppl:
                        ...
                        save_model(autoencoder, MLP_G(), MLP_D(), args)
                    else:
                        impatience += 1
                        # end training
